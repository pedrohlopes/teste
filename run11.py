...
def custom_loss(y_true,y_pred):
	...
	frame_step=512,
	fft_length=1024)
    y = tf.keras.backend.permute_dimensions(stft_data,(0,2,1))
...
dataset_val = make_dataset('/home/pedro.lopes/data/audio_data/train/augmented/val/')
...
dataset_val = tf.data.Dataset.from_generator(dataset_val,
                                                 ...
                                                 output_shapes=(tf.TensorShape(
                                                     (441000, )),
                                                                tf.TensorShape(
                                                                    (441000, ))))
dataset_train = dataset_train.cache().batch(8).map(
            mapped_function,
            num_parallel_calls=tf.data.experimental.AUTOTUNE).prefetch(
                tf.data.experimental.AUTOTUNE)
dataset_val = dataset_val.cache()batch(8).map(
            mapped_function,
            num_parallel_calls=tf.data.experimental.AUTOTUNE).prefetch(
                tf.data.experimental.AUTOTUNE)
...
with strategy.scope():
    ...
    params = {'dim': (freq_bins,sample_len),
              ...
              'pitch_shift': 0.3,
              'time_stretch': 0.3}
    model.compile(loss=tf.keras.losses.mae, optimizer= Adam(lr=8e-3, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0))
