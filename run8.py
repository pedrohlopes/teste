...
# 	frame_step=512,
# 	fft_length=1024)
    y = tf.keras.backend.permute_dimensions(y_true,(0,2,1))
    y_shape = tf.cast(tf.shape(y)[-1],tf.float64)
    pad_len = tf.math.floor(tf.math.ceil(y_shape/128)*128 - y_shape)
    pad = ([0,0],[0,0],[0,pad_len])
    y = tf.pad(y,pad,mode='constant', constant_values=0)
    y = preprocess_tf(y,X_mean,X_std)[:,0:512]
    y_split = tf.split(y,7,axis = -1)
    y = tf.concat(y_split,axis=0)
...
with strategy.scope():
    ...
    freq_bins = 512
    sample_len = 128
    l_input = Input(shape = (None,None))
#     stft_data = tf.signal.stft(l_input, 
# 	frame_length=1024, 
# 	frame_step=512,
# 	fft_length=1024)
    x_before = tf.keras.backend.permute_dimensions(l_input,(0,2,1))
    ...
    l_out_1= get_unet_mask(x,kernel_size = (7,3))
    l_out_2= get_unet_mask(x,kernel_size = (3,7))
...
def make_dataset(base_dir):
    def _generator(*args):
        ids = map(lambda x: re.findall(r"sample\_([0-9]+)\_mix\.wav", x)[0],
                  glob.glob(os.path.join(base_dir, "*_mix.wav")))

        mix_vocals = [(os.path.join(base_dir, f'sample_{_id}_mix.wav'),
                       os.path.join(base_dir, f'sample_{_id}_vocals.wav'))
                      for _id in ids]

        for idx in range(len(mix_vocals)):
            # Reading data (line, record) from the file
            mix_path, vocals_path = mix_vocals[idx]
            audio_mix, sr = af.read(mix_path)
            audio_vocal, sr = af.read(vocals_path)
...
def mapped_function(x, y):
    return tf.math.abs(tf.signal.stft(x,
                          frame_length=1024,
                          frame_step=512,
                          fft_length=1024)), tf.math.abs(tf.signal.stft(y,
                                                           frame_length=1024,
                                                           frame_step=512,
                                                           fft_length=1024))
...
dataset_train = make_dataset('/home/pedro.lopes/public_html/audios_teste_igor/')
dataset_val = make_dataset('/home/pedro.lopes/public_html/audios_teste_igor/')
...
dataset_train = dataset_train.map(
            mapped_function,
            num_parallel_calls=tf.data.experimental.AUTOTUNE).batch(8).prefetch(
...
tensorboard_callback =TensorBoard(log_dir=log_dir, histogram_freq=1,update_freq=15, profile_batch='100,200')
