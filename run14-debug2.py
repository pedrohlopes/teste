...
def custom_loss(y_true,y_pred):
    y=preprocess_audio_tf_one(y_true)
...
with tf.device('GPU:0'):
    ...
    freq_bins = 512
    sample_len = 128
    l_input = Input(shape = (441000,))
    x = preprocess_audio_tf_one(l_input)
    print(x.shape)
    l_out_1= get_unet_mask(x,kernel_size = (7,2))
    l_out_2= get_unet_mask(x,kernel_size = (2,7))
...
checkpoint = ModelCheckpoint(checkpoint_path,save_weights_only=True, monitor='val_loss', verbose=1, mode='auto',period=1)
...
with tf.device('GPU:0'):
    ...
    params = {'dim': (freq_bins,sample_len),
              ...
              'pitch_shift': 0.3,
              'time_stretch': 0.3}
    model.compile(loss=custom_loss, optimizer= Adam(lr=6e-2, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0))
