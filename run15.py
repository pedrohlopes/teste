...
def make_dataset(pattern):
    ...
    names_mix = glob(pattern + '/mix/*')
    names_vocals = glob(pattern + '/vocals/*')
    paths_tuples = [(names_mix[i], names_vocals[i]) for i in range(0, len(names_mix))] 
...
def load_audio_tf(filenames):
    ...
    audio_mix = tf.squeeze(audio_mix,axis=-1)
    buf_vocal = tf.io.read_file(
        vocal_filename, name=None
    )
    audio_vocal = tf.audio.decode_wav(buf_vocal)[0]
    audio_vocal = tf.squeeze(audio_vocal,axis=-1)
    start = tf.random.uniform(shape = [],minval=0,maxval=tf.size(audio_mix)- 441000,dtype=tf.int32)
    stop = start + 441000
    audio_mix = audio_mix[start:stop]
    audio_vocal = audio_vocal[start:stop]
...
def load_single_audio(filenames):
    ...
    audio_mix, sr = af.read(bytes.decode(mix_filename))
    audio_vocal, sr = af.read(bytes.decode(vocal_filename))
    start = np.random.randint(0,tf.size(audio_mix)- 441000)
    stop = start + 441000
    audio_mix = audio_mix[start:stop]
    audio_vocal = audio_vocal[start:stop]
    return audio_mix,audio_vocal
...
def get_train_val():
    BATCH_SIZE = 128
    dataset_train = make_dataset("/home/pedro.lopes/data/audio_data/train/train/")
    dataset_val = make_dataset("/home/pedro.lopes/data/audio_data/train/val")
...
#                      tf.data.experimental.AUTOTUNE)
    ...
    dataset_train = dataset_train.interleave(lambda x: tf.data.Dataset.from_tensor_slices([x]).map(load_single_audio),cycle_length=n_parallel,num_parallel_calls=n_parallel)
    dataset_val = dataset_val.interleave(lambda x: tf.data.Dataset.from_tensor_slices([x]).map(load_single_audio),cycle_length=n_parallel,num_parallel_calls=n_parallel)
...
with tf.device('GPU:0'):
    ...
    params = {'dim': (freq_bins,sample_len),
              ...
              'pitch_shift': 0.3,
              'time_stretch': 0.3}
    model.compile(loss=tf.keras.losses.mae, optimizer= Adam(lr=3e-2, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0))
    #model.compile(loss=tf.keras.losses.mae, optimizer= SGD(lr=1e-2, decay=1e-7, momentum=0, nesterov=True))
    
    #model.load_weights('../checkpoints/model0802.hdf5')
